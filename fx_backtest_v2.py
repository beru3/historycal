...
class FXBacktestSystemComplete:
    ...
    def decide_layer(self, spread, true_range, d5, d15, d60, sp25, sp50, tr75):
        """3層戦略の層判定"""
        if spread <= sp25 and true_range <= sp50:
            return 'BASE'
        if spread <= sp50 and d5 and d15 and d60:
            return 'EXPAND'
        return 'ATR'

    def get_layer_sl_tp(self, layer, atr14):
        """層別SL/TP設定"""
        if layer == 'BASE':
            return 10, 20
        if layer == 'EXPAND':
    ...
    # def backtest_single_day(self, entry_data):

    def backtest_single_day(self, entry_data):
        """1日分のバックテスト実行（ストップロス機能 + 3層戦略付き）"""
        date_obj = entry_data['date']
        date_str = entry_data['date_str']
        df_entries = entry_data['data']
        
        logger.info(f"📅 バックテスト実行: {date_str} ({len(df_entries)}エントリーポイント)")
        
        daily_results = []
        processed_currencies = {}  # 通貨ペア別にキャッシュ
        ...
        for idx, entry in df_entries.iterrows():
            try:
                ...
                logger.info(f"  💱 処理中: {currency_pair} {direction} {entry_time_str}-{exit_time_str}")
                
                # 過去データファイルを取得（改良版を使用）
                zip_path = self.find_historical_data_file_improved(currency_pair, date_obj)
                if not zip_path:
                    logger.warning(f"    ❌ 過去データなし: {currency_pair}")
                    continue
                
                # キャッシュチェック（同じ通貨ペア・日付の場合は再利用）
                ...
                if cache_key not in processed_currencies:
                    ...
                    # 過去データを読み込み（3層戦略データ付き）
                    df_historical = self.safe_csv_read_from_zip(zip_path, date_obj)
                    if df_historical is None or df_historical.empty:
                        logger.warning(f"    ❌ データ読み込み失敗: {currency_pair}")
                        processed_currencies[cache_key] = None
                        continue
                    
                    # データの基本情報をログ出力
                    logger.info(f"    📊 読み込みデータ: {len(df_historical)}行")
                    if 'timestamp' in df_historical.columns:
                        min_time = df_historical['timestamp'].min()
                        max_time = df_historical['timestamp'].max()
                        logger.info(f"    ⏰ データ時間範囲: {min_time} ～ {max_time}")
                    
                    # キャッシュに保存
                    processed_currencies[cache_key] = df_historical
                    logger.info(f"    📦 キャッシュ保存: {cache_key}")
                else:
                    # キャッシュから取得
                    df_historical = processed_currencies.get(cache_key)
                    if df_historical is None:
                        logger.warning(f"    ❌ キャッシュデータなし: {cache_key}")
                        continue
                    logger.debug(f"    📦 キャッシュから取得: {cache_key}")
                
                # エントリー時刻の市場条件を取得（3層戦略用）
                entry_datetime = f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]} {entry_time_str}"
                market_conditions = self.get_entry_market_conditions(df_historical, entry_datetime)
                
                # 3層戦略での層判定（Shortの場合は方向フラグを反転）
                if direction.upper() in ['SHORT', 'SELL']:
                    # Shortトレードでは方向フラグを反転（下降トレンドを求める）
                    dir_5m = not market_conditions['dir_5m']
                    dir_15m = not market_conditions['dir_15m']
                    dir_1h = not market_conditions['dir_1h']
                else:
                    # Longトレードではそのまま使用
                    dir_5m = market_conditions['dir_5m']
                    dir_15m = market_conditions['dir_15m']
                    dir_1h = market_conditions['dir_1h']
                
                layer = self.decide_layer(
                    market_conditions['spread'],
                    market_conditions['true_range'],
                    dir_5m,
                    dir_15m,
                    dir_1h,
                    market_conditions['spread_q25'],
                    market_conditions['spread_q50'],
                    market_conditions['true_range_q75']
                )
                
                # 層別のSL/TP設定を取得
                sl_pips, tp_pips = self.get_layer_sl_tp(layer, market_conditions['atr14'])
                
                # 一時的にSL/TP設定を上書き
                original_sl = self.stop_loss_pips
                original_tp = self.take_profit_pips
                self.stop_loss_pips = sl_pips
                self.take_profit_pips = tp_pips
                
                logger.info(f"    🎯 層判定: {layer} (SL:{sl_pips}pips, TP:{tp_pips}pips)")
                
                # エントリー時刻の価格を取得
                logger.info(f"    🔍 エントリー時刻検索: {entry_datetime}")
                
                entry_price, actual_entry_time = self.get_price_at_time_improved(
                    df_historical, entry_datetime, direction
                )
                
                if entry_price is None:
                    logger.warning(f"    ❌ エントリー価格取得失敗: {entry_datetime}")
                    # SL/TP設定を復元
                    self.stop_loss_pips = original_sl
                    self.take_profit_pips = original_tp
                    continue
                
                logger.info(f"    ✅ エントリー価格: {entry_price} @ {actual_entry_time}")
                
                # エグジット時刻を設定
                exit_datetime = f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]} {exit_time_str}"
                
                # ストップロス・テイクプロフィット監視（層別設定適用）
                logger.info(f"    🎯 ストップロス監視開始 ({layer}層)")
                exit_result = self.monitor_position_with_stop_loss(
                    df_historical, entry_datetime, exit_datetime, entry_price, direction, currency_pair
                )
                ...
                if exit_result is None:
                    logger.warning(f"    ❌ ストップロス監視失敗")
                    continue
                
                exit_price = exit_result['exit_price']
                actual_exit_time = exit_result['actual_exit_time']
                exit_reason = exit_result['exit_reason']
                max_favorable_pips = exit_result['max_favorable_pips']
                max_adverse_pips = exit_result['max_adverse_pips']
                
                logger.info(f"    ✅ エグジット: {exit_price} @ {actual_exit_time} ({exit_reason})")
                
                # 時刻の妥当性チェック
                if actual_entry_time == actual_exit_time:
                    logger.warning(f"    ⚠️  エントリーとエグジットが同時刻: {actual_entry_time}")
                    # この場合はスキップまたは少し時間をずらす
                    continue
                ...
                trade_result = {
                    ...
                    'max_favorable_pips': max_favorable_pips,
                    'max_adverse_pips': max_adverse_pips,
                    'layer': layer,  # 3層戦略情報を追加
                    'sl_pips': sl_pips,
                    'tp_pips': tp_pips,
                    'score': entry.get('実用スコア', 0),
                    'short_win_rate': entry.get('短期勝率', 0),
                    'mid_win_rate': entry.get('中期勝率', 0),
                    'long_win_rate': entry.get('長期勝率', 0)
                }
                
                daily_results.append(trade_result)
                
                # ログ出力を詳細化
                status_emoji = "🎯" if exit_reason == "TAKE_PROFIT" else "🛑" if exit_reason == "STOP_LOSS" else "⏰"
                logger.info(f"    ✅ {status_emoji} {currency_pair} {direction} [{layer}]: {pips:.1f}pips ({result}) - {exit_reason}")
        ...
        logger.info(f"  ✅ 完了: {len(daily_results)}取引分析")
