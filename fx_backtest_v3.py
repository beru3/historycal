...
class FXBacktestSystemComplete:
    ...
    def decide_layer(self,
                    ...
                    tr50: float,
                    atr14: float,
                    atr14_median: float) -> str:
        """
        BASE / EXPAND / ATR を返す
        """
        # ── BASE ── 低スプレッド & 低ボラ
        if (spread <= sp35) and (true_range <= tr50):
            return "BASE"

        # ── EXPAND ── 中庸スプレッド以下 + 方向一致 + 勢い（ATR高）
        if (spread <= sp40) and dir_5m and dir_15m and dir_1h and (atr14 > atr14_median):
            return "EXPAND"
    ...
    def get_layer_sl_tp(self, layer: str, atr14: float) -> tuple[int, int]:
        """
        戻り値: (stop_loss_pips, take_profit_pips)
        """
        if layer == "BASE":
            return 10, 18                # RR 1 : 1.8

        if layer == "EXPAND":
            return 12, 30                # RR 1 : 2.5

        # layer == "ATR"
        sl = int(atr14 * 1.3)            # ATR × 1.3
        return sl, sl * 2

    @staticmethod
    def _calc_day_thresholds(df_day: pd.DataFrame) -> dict:
        """
        その日のスプレッド／ボラ分位点を辞書で返す
        """
        return {
