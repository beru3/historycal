...
"""
FX自動エントリー・イグジット監視スクリプト
- CSVファイルからエントリー・イグジットの時間を読み込む
- 指定された時間にSaxo Bank APIを使用してリアルタイムレートを取得
...
def parse_time_string(time_str):
    """時刻文字列（HH:MM:SS）をdatetimeオブジェクトに変換"""
    now = datetime.now()
...
def schedule_entry_points(df, output_file):
    """エントリーポイントの監視をスケジューリング"""
    global entry_exit_data, trading_complete
    
    entry_times = {}  # 既にスケジュール済みの時間を記録
    
    # エントリータイムとイグジットタイムを現在の日付と結合
    for index, row in df.iterrows():
        try:
            ...
            exit_time = parse_time_string(exit_time_str)
            
            # 現在時刻と比較して、既に過ぎている時間はスキップ
            now = datetime.now()
            if entry_time < now:
            ...
            # エントリー・イグジットデータを保存
            entry_key = f"{no}_{currency_pair}"
            ...
            # エントリー時間に実行するジョブを登録
            entry_time_id = f"{entry_time_str}_{currency_pair}_{no}"
            ...
            # イグジット時間に実行するジョブを登録
            exit_time_id = f"{exit_time_str}_{currency_pair}_{no}"
            ...
            logger.error(f"スケジュール登録エラー (行 {index+1}): {e}")
    
    # 全てのジョブが完了したら実行を終了するタイマー設定
    # 最も遅いイグジット時間を見つける
    latest_exit_time = None
    for key, data in entry_exit_data.items():
        exit_time = parse_time_string(data["exit_time"])
    ...
    if latest_exit_time:
        ...
        logger.warning("スケジュール可能な時間がありませんでした")
        return False
...
def main():
    # コマンドライン引数の解析
    parser = argparse.ArgumentParser(description="FX自動エントリー・イグジット監視ツール")
    ...
    # 出力ファイル名設定
    output_file = args.output if args.output else os.path.join(DATA_DIR, f"fx_results_{date_str}.csv")
    
    logger.info("============================================")
    logger.info("=== FX自動エントリー・イグジット監視ツール ===")
    logger.info("============================================")
    ...
    try:
        ...
        # 3. エントリーポイントのスケジューリング
        if schedule_entry_points(entry_points, output_file):
            # 4. スケジューラ実行
            run_scheduler()
        else:
            logger.error("スケジュール設定に失敗しました")
            sys.exit(1)
