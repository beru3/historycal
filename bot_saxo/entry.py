...
def get_historical_price(currency_pair, timestamp_iso, uic=None):
    ...
    # InfoPricesAsyncサービスを使用して過去のレートを取得
    try:
        # ステップ1: 非同期リクエストを作成
        url = f"{GATEWAY_URL}/trade/v1/infoprices/subscriptions"
        
        # ISOタイムスタンプからミリ秒を抜くために固定フォーマットを使用
        timestamp_parts = timestamp_iso.split('.')
        clean_timestamp = timestamp_parts[0] + 'Z' if len(timestamp_parts) > 1 else timestamp_iso
        
        data = {
            "Arguments": {
                "AssetType": "FxSpot",
                "Uic": uic,
                "FieldGroups": ["PriceInfo", "DisplayAndFormat"],
                "FromDateTime": clean_timestamp  # ISO8601形式のタイムスタンプ
            },
            "ContextId": f"hist_{currency_pair}_{timestamp_iso.replace(':', '').replace('-', '')}"
        }
        ...
        resp.raise_for_status()
        ref_id = resp.headers.get('X-Request-ID')
        
        if not ref_id:
            logger.error("非同期リクエストからリクエストIDを取得できませんでした")
            return None
        
        # ステップ2: 結果を取得（リファレンスIDを使用）
        # 数回のリトライを許容する
        for _ in range(5):
            try:
                time.sleep(1)  # APIのレート制限を考慮して待機
                result_url = f"{GATEWAY_URL}/trade/v1/infoprices"
                params = {"ReferenceId": ref_id}
                result_resp = requests.get(result_url, headers=headers, params=params)
                ...
                if result_resp.status_code == 200:
                    ...
                    if "Quote" in data:
                        ...
                        return {
                            "bid": quote.get("Bid"),
                            "ask": quote.get("Ask"),
                            "mid": (float(quote.get("Bid", 0)) + float(quote.get("Ask", 0))) / 2,
                            "timestamp": timestamp_iso
                        }
                elif result_resp.status_code == 202:  # まだ処理中
                    continue
                else:
                    logger.error(f"履歴レート取得エラー: {result_resp.status_code}, {result_resp.text}")
                    break
            except Exception as e:
                logger.error(f"履歴レート取得中の例外: {e}")
                break
        
        logger.warning(f"タイムアウト: {currency_pair} の {timestamp_iso} のレート取得に失敗")
        return None
        ...
        for index, row in df.iterrows():
            try:
                ...
                result = {
                    ...
                    "Entry_Timestamp": entry_iso_timestamp,
                    "Exit_Bid": exit_rate["bid"],
                    "Exit_Ask": exit_rate["ask"],
                    "Exit_Mid": exit_rate["mid"],
                    "Exit_Timestamp": exit_iso_timestamp,
                    "Pips": pips,
...
def save_results(output_file):
    ...
    try:
        ...
        if any(isinstance(r.get('Pips'), (int, float)) for r in results_data):
            ...
            valid_results = [r for r in results_data if isinstance(r.get('Pips'), (int, float))]
