...
# -*- coding: utf-8 -*-
"""
fx_backtest_fixed_complete.py - FXエントリーポイント バックテストシステム（完全修正版）
...
class FXBacktestSystemComplete:
    ...
    def __init__(self, stop_loss_pips=None, take_profit_pips=None):
        ...
        logger.info("FXバックテストシステム（完全修正版 + ストップロス機能）を初期化しました")
    ...
    def backtest_single_day(self, entry_data):
        ...
        for idx, entry in df_entries.iterrows():
            try:
                ...
                # ストップロス・テイクプロフィット監視
                if self.stop_loss_pips or self.take_profit_pips:
                    logger.info(f"    🎯 ストップロス監視開始")
                    ...
                    # 通常のエグジット（ストップロスなし）
                    logger.info(f"    🔍 エグジット時刻検索: {exit_datetime}")
                    
                    # エグジット時は逆方向の価格を使用
                    exit_direction = 'SELL' if direction.upper() in ['LONG', 'BUY'] else 'BUY'
                    exit_price, actual_exit_time = self.get_price_at_time_improved(
                        df_historical, exit_datetime, exit_direction
                    )
                    
                    if exit_price is None:
                        logger.warning(f"    ❌ エグジット価格取得失敗: {exit_datetime}")
                        continue
                    ...
                    logger.info(f"    ✅ エグジット価格: {exit_price} @ {actual_exit_time}")
                ...
                logger.info(f"    ✅ {status_emoji} {currency_pair} {direction}: {pips:.1f}pips ({result}) - {exit_reason}")
    ...
    def run_backtest(self):
        """バックテスト実行（エラー耐性強化版）"""
        logger.info("=" * 80)
        logger.info("🚀 FXバックテスト開始（エラー耐性強化版）")
    ...
    def generate_html_report(self):
        ...
        <head>
            ...
            <title>FXバックテスト結果レポート（完全修正版）</title>
            <style>
    ...
    def generate_charts(self):
        ...
        try:
            ...
            fig.suptitle('FX Backtest Performance Analysis (Complete Fixed)', fontsize=16, fontweight='bold')
...
def main():
    """メイン実行関数"""
    try:
        print("🚀 FXバックテストシステム（完全修正版）を開始します...")
        ...
        print("  - backtest_results_complete_*.csv : 全取引詳細データ")
        print("  - backtest_report_complete_*.html : 美しいHTMLレポート")
        print("  - backtest_charts_complete_*.png : パフォーマンスチャート")
